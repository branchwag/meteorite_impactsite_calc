...
**Scenario 2 — I have a dark flight point.** Predicts *where to look on radar*: given the fireball end time, a dark-flight point (lat/lon/altitude), and an entry trajectory, it first continues the ballistic path 0.5 s past the dark-flight point at a fixed 12 km/s (6 km of path — speed is not user-entered). The entry/elevation angle splits that path linearly between vertical drop and horizontal travel (90° = 6 km straight down, 45° = 3 km down + 3 km along the azimuth, 0° = all horizontal). From the resulting point it projects the descent downward (optionally wind-corrected) for an assumed 100 g chondrite and reports the lat/lon and clock time the rock passes each lower altitude, down to the ground. If no azimuth is entered (default 0), the heading is treated as unknown and the result is a wide circular search area. Type, size, number of rocks, and ground location are **not** computed here — they require an actual radar hit, which is Scenario 1.
